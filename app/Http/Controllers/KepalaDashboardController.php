...
class KepalaDashboardController extends Controller
{
    public function index()
    {
        ...
        // Daftar petugas
        $petugasAktif = User::where('role', 'petugas')->orderBy('name')->get();

        // RL 5.1 — Morbiditas: semua diagnosa dengan breakdown L/P
        // Kelompok umur standar RL 5.1
        ...
        foreach ($rawRl51 as $row) {
            ...
            $umur = $row->ttl ? (int) floor((time() - strtotime($row->ttl)) / (365.25 * 86400)) : null;
            if ($umur !== null) {
                if ($umur < 1)       $gol = '<1 thn';
                ...
                if (in_array($jk, ['L','P'])) {
                    ...
                    $rl51Rows[$key]['kunjungan_' . $jk]   += $row->total;
                    $rl51Rows[$key]['kunjungan_total']     += $row->total;
                }
            }
        }
